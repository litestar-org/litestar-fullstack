...
class TestEmailService:
    ...
    async def test_send_template_email_success(self, email_service, mock_smtp):
        ...
        email_service.jinja_env.get_template = Mock(side_effect=[
            mock_html_template,
            mock_text_template
        ])
    ...
    @pytest.mark.asyncio
    async def test_send_verification_email_with_template(self, email_service, mock_user, mock_verification_token, mock_smtp):
        ...
        assert call_args[1]["context"]["verification_url"] == "https://app.test.com/verify-email?token=test-verification-token-123"
