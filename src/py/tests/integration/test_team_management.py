...
from tests.factories import UserFactory, TeamFactory, TeamMemberFactory, create_user_with_team, create_team_with_members
...
class TestTeamCRUD:
    ...
    async def test_update_team_as_member(
        ...
        async with AsyncTestClient(app=app) as client:
            token = await self._login_user(client, member)
...
class TestTeamMemberManagement:
    ...
    async def test_add_member_to_team_success(
        ...
        result = await session.execute(
            select(m.TeamMember).where(
                m.TeamMember.team_id == team.id,
                m.TeamMember.user_id == new_member.id
            )
        )
    ...
    async def test_remove_member_from_team_success(
        ...
        result = await session.execute(
            select(m.TeamMember).where(
                m.TeamMember.team_id == team.id,
                m.TeamMember.user_id == member_to_remove.id
            )
        )
        membership = result.scalar_one_or_none()
...
class TestTeamPermissions:
    """Test team permission enforcement."""
    ...
    async def test_team_admin_permissions(
        ...
        async with AsyncTestClient(app=app) as client:
            token = await self._login_user(client, admin)
    ...
    async def test_non_member_permissions(
        ...
        async with AsyncTestClient(app=app) as client:
            token = await self._login_user(client, non_member)
...
class TestTeamIntegration:
    ...
    async def test_complete_team_lifecycle(
        ...
        async with AsyncTestClient(app=app) as client:
            ...
            response = await client.post(
                "/api/teams",
                json={
                    "name": "Full Lifecycle Team",
                    "description": "Test team for complete workflow"
                },
                headers={"Authorization": f"Bearer {token}"}
            )
            assert response.status_code == 201
    ...
    async def test_team_security_isolation(
        ...
        async with AsyncTestClient(app=app) as client:
            token1 = await self._login_user(client, owner1)
            token2 = await self._login_user(client, owner2)
