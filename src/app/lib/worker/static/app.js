...
const queue_view = function (data, queue_name) {
  ...
  return h("div", [
    h("hgroup", [h("h1", "Queue"), h("h2", queue_name)]),
    h("table", [
      h("thead", [
        h("tr", [h("th", "Active"), h("th", "Queued"), h("th", "Scheduled")]),
      ]),
      h(
        "tbody",
        h("tr", [
    ...
    h("table", { attrs: { role: "grid" } }, [
      ...
      h(
        "tbody",
        Object.entries(queue.workers).map(([name, worker]) =>
          h("tr", [
            h("td", name),
...
const job_view = function (data, queue_name, job_key) {
  ...
  return h("div", [
    h("hgroup", [h("h1", "Job"), h("h2", job_key)]),
    h("grid", buttons),
    h(
      "figure",
      h("table", [
...
let routes = {
  "/worker/": { view: home_view, data: "/queues" },
