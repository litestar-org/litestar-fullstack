...
class ViteConfig(BaseModel):
    ...
    protocol: str = "http"
    port: int = 3000
    vite_run_command: list[str] = ["npm", "run", "dev"]
    vite_build_command: list = ["npm", "run", "build"]
...
class ViteAssetLoader:
    ...
    def parse_manifest() -> dict[str, Any]:
        ...
        if not config.hot_reload:
            with open(Path(config.static_dir / "manifest.json")) as manifest_file:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    Path(config.static_dir / "manifest.json"),
                ) from exc
        return manifest
    ...
    def generate_vite_asset(self, path: str, scripts_attrs: dict[str, str] | None = None) -> str:
        ...
        if path not in self._manifest:
            raise RuntimeError("Cannot find %s in Vite manifest at %s", path, Path(config.static_dir / "manifest.json"))
